...
class RestAdapter {
	...
	def renderMapListRest(list,  MaprestFormat f, String root = null){
		
		def resultMap;
		def scrubbedMapList = list.collect{ o ->
			...
			switch (f) {
				...
				case MaprestFormat.JSON:
					...
					return scrubObjectForJson(map)
			}
			
		}
		if (root) {
			resultMap = ["${root}": scrubbedMapList]
		}else{
			resultMap = scrubbedMapList
		}
		
		
		api.render(this, resultMap as JSON)
	}
